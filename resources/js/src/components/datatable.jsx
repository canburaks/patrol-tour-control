...
/*
NOT: Data must be an array contains object at least name and link values
data = [name, ..., ..., link]
*/
const Datatable = ({ headers, records }) => {
	console.log("datatable", headers, records)
	return (
		<div className="container px-4 mx-auto">
			<div className="pt-6 bg-white shadow rounded">
				<div className="px-6 border-b">
					<div className="flex flex-wrap items-center mb-6">
						<h3 className="text-xl font-bold">Abonelerinizin listesi</h3>
						<a
							...
							<tr className="text-xs text-gray-500 text-left">
								{headers.map((h, i) => {
									return (
										<th
											key={`${h}-${i}`}
											className="flex items-center pl-6 py-4 font-medium">
							...
							{records.map((record, index) => {
								return (
									<tr key={index} className="text-xs bg-gray-50">
										...
										{record.slice(1, record.length - 1).map(rk => {
											;<td key={rk[1]} className="font-medium">
												record[i]
											</td>
										})}
										...
										<td>
											<Link
												to={record[headers.length - 1]}
												className="inline-block py-1 px-2 text-white bg-green-500 rounded-full">
												Sinyalleri incele
											</Link>
										</td>
									</tr>
